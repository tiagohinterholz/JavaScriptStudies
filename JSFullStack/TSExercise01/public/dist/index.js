...
function addCrewMember(member, spaceship) {
    if (spaceship.crew.length >= spaceship.crewLimit) {
        ...
        alert(`${member} foi adicionado a tribulação!`);
    }
}
function sendInMission(spaceship) {
    if (spaceship.inMission) {
        alert(`${spaceship.name} já em missão!!`);
    }
    else if (spaceship.crew.length < Math.floor(spaceship.crewLimit / 3)) {
        alert(`${spaceship.name} não pode ser enviada por falta de tripulação!!`);
    }
    else {
...
function firstMenuOption() {
    ...
    const pilot = prompt('Qual o nome do piloto a ser registrado?');
    const crewLimit = Number(prompt('Quantos tripulantes a nave suporta?'));
    const confirmation = confirm(`Confirma o registo da nave ${name}\nPilot: ${pilot}\nLimite da tripulação: ${crewLimit}`);
    if (confirmation) {
